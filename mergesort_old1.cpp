...
int MergeSort::unionHijos(int M,size_t B,std::ifstream& archivo) const{
    ...
    while (true) {
            ...
            if(revision_final[i]!=-1){
                ...
                if(indice_numero > ultimo[i]){
                    ...
                    if(revision_final[i]==1){
                        ...
                        int pos_nueva = (inicios[i]+ultimo[i]*trozos_subidos[i]); // TODO: revisar aritmetica
                        int cantidad_a_llevar = ultimo[i];

                        //Si ya estoy en el último bloque del arreglo a subir
                        if( pos_nueva + (ultimo[i]) >=
                            this->hijos[i].getInicio() +
                            (this->hijos[i].getLargo()/sizeof(uint64_t))) {
        ...
        if (min_idx == -1) {
            ...
            std::fstream archivoFuera(  filename,
            ...
            archivoFuera.seekp(inicio * sizeof(uint64_t), std::ios::beg);
            IOs+=escritura_bloques(B,
                                   buffer_final.size(),
                                   buffer_final, archivoFuera);
            archivoFuera.close();
            break;
        }
        ...
        if( buffer_final.size() >=
            tamaño_agregar/sizeof(uint64_t)) {   
